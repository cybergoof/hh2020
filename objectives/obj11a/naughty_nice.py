...
class Block():
    ...
    def dump_doc(self, doc_no):
        ...
        # Changed so that files would go into a directory
        with open(os.path.join("docs", filename), 'wb') as fh:
...
def getNonces(hexNonce):
    lower_hex = hexNonce[-8:]
    upper_hex = hexNonce[:len(hexNonce)-8]
    lower_nonce = int(lower_hex, 16)
    upper_nonce = int(upper_hex, 16)
    # print(decNonce_1.bit_length(), decNonce_2.bit_length())
    return lower_nonce, upper_nonce


def combine(lower_num, higher_num):
    lower_string = format(lower_num, '08X')
    upper_string = format(higher_num, '08X')
...
if __name__ == '__main__':
    ...
    # Commenting out so we can start building our own
    if (False):
        c1 = Chain()
        for i in range(9):
            block_data = {}
        ...
        # Just a copy of the code that was provided
    ...
    nonces = []
    # Need to extract all the nonces

    # need to split all the nonces

    # Run them through liston's to predict?

    myprng = mt19937(0)

    seed_array = []

    available_seeds = len(c2.blocks)
    nonce_array = []
    desired_blockchain_index = 13000

    # we will build the seed list
    for block in c2.blocks:
        # Convert the seed.
        lower, upper = getNonces(format(block.nonce, '016X'))

        seed_array.append(lower)
        seed_array.append(upper)
        test_nonce = combine(lower, upper)
        real_nonce = format(block.nonce, '016X')
        if real_nonce != test_nonce:
            print("[ERROR] Block is bad for {} ".format(block.index))
#        else:
#            print("Block is good for {} ".format(block.index))
        nonce_array.append(format(block.nonce, '016X'))

    # We will build the prediction system
    for i in range(mt19937.n):
        myprng.MT[i] = untemper(seed_array[i])


    # Now, I need to predict through to the end of the blockchain (and match)

    # then, I make the next guesses until we hit index 13000

    predicted_numbers = []
    block_chain_index = 0
    for block in c2.blocks[int(mt19937.n/2):]:
        lower_predict = myprng.extract_number()
        upper_predict = myprng.extract_number()
        new_num = combine(lower_predict, upper_predict)
        hex_is = format(block.nonce, '016X')
        found = next((x for x in nonce_array if x == new_num), None)
        if (found==None):
            print("Not found")
        if combine(lower_predict, upper_predict) != format(block.nonce, '016X'):
            print("Something wrong: block.nonce %")

#        predicted_numbers.append(next_predict)

    for i in range(c2.blocks[-1].index+1,130001):
        low_predict = myprng.extract_number()
        high_predict = myprng.extract_number()
        combined_value = combine(low_predict, high_predict)
        print("Index {} nonce is {}".format(i, combined_value))

    print("Answer should be 57066318F32F729D")

    # Now, we go and start predicting
